...
Remain Inbox-oriented. Do not recursively audit all contexts, links, and schema rules; use `lint-engram` for that.

## Procedure

1. Run `python Tools/engram.py status` and capture its exact exit status and output. If unavailable or failing, report the limitation and continue with the targeted checks; do not invent status data.
2. Target-scan `Engram/Inbox/` and `Engram/Health/Inbox/` metadata for review candidates, then cross-check them against `Engram/INDEX.md`. Never rely only on status encoded in the index.
...
6. Read `Engram/.review-state.md` and diff the current candidates against its last-surfaced list: mark items seen in a prior run and still unresolved as "still open" instead of presenting them as new, so unresolved items do not get re-flagged fresh on every run.
...
11. Apply approved actions through `sync-engram` semantics. Preserve every Raw Capture payload; processing may change metadata and add links only.
12. After presenting the batch, update `Engram/.review-state.md` with today's date and the current surfaced-item list. This is non-semantic bookkeeping and does not require separate approval.
