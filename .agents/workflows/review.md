...
1. Load and follow `.agents/skills/review-brain/SKILL.md`.
2. Default to Inbox review unless the user names another scope.
3. Run `python Tools/brain.py status` and report an exact failure if it is unavailable.
4. Target-scan Inbox metadata, cross-check candidates against `Brain/INDEX.md`, and read only nearby relevant context. Never rely on index status alone.
